...
export const Layout: React.FC = () => {
  ...
  const profileImageUrl = useMemo(() => {
    if (!user?.profile_image) return defaultAvatar;

    const profileImage = user.profile_image.trim();

    // Si jamais la valeur est déjà une URL absolue, on la normalise
    // pour forcer le bon domaine/base URL.
    if (/^https?:\/\//i.test(profileImage)) {
      try {
        const url = new URL(profileImage);
        return `${storageBaseUrl}${url.pathname}`;
      } catch {
        return profileImage;
      }
    }
