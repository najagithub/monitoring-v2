...
const API_BASE = ((import.meta as any).env?.VITE_API_URL as string | undefined)?.replace(/\/api\/v1\/?$/, '') || '';
                        ...
                        {user.is_active && user.user_provider && (
                          <div className="flex items-center justify-center">
                            {user.user_provider.internet_status ? (
                              <Wifi className="w-5 h-5 text-green-500" strokeWidth={3} />
                            ) : (
                              <WifiOff className="w-5 h-5 text-red-500" strokeWidth={3} />
                            )}
                          </div>
                        )}
                      </div>
...
const UserDetails: React.FC<{
  ...
  const initialOidIn = user.user_provider?.oid_byte_in || user.user_provider?.oid_in || '';
  const initialOidOut = user.user_provider?.oid_byte_out || user.user_provider?.oid_out || '';
